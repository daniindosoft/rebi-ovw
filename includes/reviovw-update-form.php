...
<?php global $wpdb; $tbl_form = $wpdb->prefix . "rebiovw_form"; $tbl_form_line = $wpdb->prefix . "rebiovw_form_line"; $tbl_pesanan = $wpdb->prefix . "rebiovw_pesanan"; if (isset($_POST['delete'])) {$wpdb->query($wpdb->prepare("DELETE FROM $tbl_form WHERE id = %s", $_GET['id'])); $wpdb->query($wpdb->prepare("DELETE FROM $tbl_form_line WHERE id_form = %s", $_GET['id'])); $wpdb->query($wpdb->prepare("DELETE FROM $tbl_pesanan WHERE id_form = %s", $_GET['id'])); $message ="Form telah dihapus"; header('location:'.admin_url('admin.php?page=rebiovw_list_form&id='.$id).'&msg='.$message); } if (isset($_POST['addform'])) {$title = $_POST['title']; $hp = $_POST['header_pesan']; $fp = $_POST['footer_pesan']; $wpdb->update($tbl_form, array('title' => $title, 'header_pesan' => $hp, 'footer_pesan' => $fp, ), array('id' => $_GET['id'] ) ); $title_elemen = $_POST['judul_elemen']; $type_elemen = $_POST['type_elemen']; $lebar = $_POST['lebar']; $for = $_POST['format']; for ($i=0; $i < count($title_elemen); $i++) {$format = $for[$i]; $elemen = $type_elemen[$i].' '.@$_POST['group'][$i]; $wpdb->update($tbl_form_line, array('judul_elemen' => $title_elemen[$i], 'type_elemen' => $elemen, 'lebar' => $lebar[$i], 'format' => $format ), array('id' => $_POST['id_form_line'][$i]) ); } $message ="Form telah dibuat"; } $data = $wpdb->get_row( "SELECT * FROM $tbl_form where id=".$_GET['id']); ?>
