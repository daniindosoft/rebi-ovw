...
global $wpdb; $tbl_template = $wpdb->prefix . "rebiovw_template"; $tbl_tombol = $wpdb->prefix . "rebiovw_tombol"; $tbl_template_line = $wpdb->prefix . "rebiovw_template_line"; $tbl_setting = $wpdb->prefix . "rebiovw_setting"; $tbl_form = $wpdb->prefix . "rebiovw_form"; $nowa = $wpdb->get_row( "SELECT * FROM $tbl_setting"); $rows = $wpdb->get_row( $wpdb->prepare( "SELECT *, t.id_form as idn, t.title as judul_formnya FROM $tbl_template as t join $tbl_template_line as tl on t.id = tl.id_template left join $tbl_tombol as btn on btn.id = t.id_tombol where t.posisi ='".$hook."' and tl.produk_id = %d", get_the_ID() ) ); if (!empty($rows->btn_text)) {$header = $wpdb->get_row( $wpdb->prepare("SELECT * FROM $tbl_form where id = %d", $rows->idn)); rebiovw_include_icon(); $urlcode = rebiovwContentUrlEncode($rows->des); if ($rows->id_form >= 1) {rebiovw_style($hook); rebiovw_modal($hook, $rows->btn_text, $rows->idn, $rows->css, $rows->judul_formnya, $rows->btn_text, rebiovwContentUrlEncode($header->header_pesan), rebiovwContentUrlEncode($header->footer_pesan), $nowa->nowa); }else{echo '<a style="'.$rows->css.'" href="https://api.whatsapp.com/send?phone='.$nowa->nowa.'&text='.$urlcode.'" target="_blank" class="button button-success"> <i class="fab fa-whatsapp"></i> '.$rows->btn_text.' </a>'; } } }
